...
namespace StringHelper
{
    ...
    {
        return v.length() > 0 && strcmp(v.c_str(), pgm2Str(esp_signer_pgm_str_19 /* "true" */)) == 0;
    }
...
namespace URLHelper
{
    ...
    inline bool addParam(MB_String &url, PGM_P key, const MB_String &val, bool &hasParam, bool allowEmptyValue = false)
    {
        ...
        if (_key[0] != '?' && _key[0] != '&')
            url += !hasParam ? esp_signer_pgm_str_28 /* "?" */ : esp_signer_pgm_str_29 /* "&" */;

        if (_key[_key.length() - 1] != '=' && _key.find('=') == MB_String::npos)
            _key += esp_signer_pgm_str_30; // "="
    ...
    {
        url = esp_signer_pgm_str_32; // "https://"
        url += host;
    }

    inline void parse(MB_FS *mbfs, const MB_String &url, struct esp_signer_url_info_t &info)
    {
        ...
        int p1 = 0;
        int x = sscanf(url.c_str(), pgm2Str(esp_signer_pgm_str_33), host, uri);
        x ? p1 = 8 : x = sscanf(url.c_str(), pgm2Str(esp_signer_pgm_str_34), host, uri);
        x ? p1 = 7 : x = sscanf(url.c_str(), pgm2Str(esp_signer_pgm_str_35), host, uri);

        size_t p2 = 0;
        if (x > 0)
        {
            p2 = MB_String(host).find(pgm2Str(esp_signer_pgm_str_28), 0);
            if (p2 != MB_String::npos)
                x = sscanf(url.c_str() + p1, pgm2Str(esp_signer_pgm_str_36), host, uri);
        }

        if (strlen(uri) > 0)
        {
            p2 = MB_String(uri).find(pgm2Str(esp_signer_pgm_str_37), 0);
            if (p2 != MB_String::npos)
                x = sscanf(uri + p2 + 5, pgm2Str(esp_signer_pgm_str_38), auth);
        }
...
namespace JsonHelper
{
    ...
    inline bool parseChunk(MB_String &val, const MB_String &chunk, const MB_String &key, int &pos)
    {
        ...
        MB_String token = esp_signer_pgm_str_3; // "\""
        ...
        {
            size_t p2 = chunk.find(MB_String(esp_signer_pgm_str_7 /* ":" */).c_str(), p1 + _key.length());
            if (p2 != MB_String::npos)
    ...
    inline void addBoolString(FirebaseJson *json, PGM_P key, const MB_String &val, bool &flag)
    {
        ...
        {
            if (isJsonPath(key))
                json->set(pgm2Str(key), strcmp(val.c_str(), pgm2Str(esp_signer_pgm_str_19)) == 0 ? true : false);
            else
                json->add(pgm2Str(key), strcmp(val.c_str(), pgm2Str(esp_signer_pgm_str_19)) == 0 ? true : false);
            flag = true;
        }
    }
...
namespace Base64Helper
{
    ...
    inline unsigned char *creatBase64EncBuffer(MB_FS *mbfs, bool isURL)
    {
        ...
        strcpy_P((char *)base64EncBuf, (char *)esp_signer_base64_table);
        if (isURL)
        {
            base64EncBuf[62] = '-';
            base64EncBuf[63] = '_';
    ...
    inline unsigned char *creatBase64DecBuffer(MB_FS *mbfs)
    {
        ...
        for (size_t i = 0; i < sizeof(esp_signer_base64_table) - 1; i++)
            base64DecBuf[esp_signer_base64_table[i]] = (unsigned char)i;
        base64DecBuf['='] = 0;
        return base64DecBuf;
    }

    template <typename T = uint8_t>
    inline bool writeOutput(MB_FS *mbfs, esp_signer_base64_io_t<T> &out)
    {
    ...
    template <typename T = uint8_t>
    inline bool setOutput(MB_FS *mbfs, uint8_t val, esp_signer_base64_io_t<T> &out, T **pos)
    {
        if (out.outT)
        {
    ...
    template <typename T>
    inline bool decode(MB_FS *mbfs, unsigned char *base64DecBuf, const char *src, size_t len, esp_signer_base64_io_t<T> &out)
    {
    ...
    inline bool encodeLast(MB_FS *mbfs, unsigned char *base64EncBuf, const unsigned char *in, size_t len,
                           esp_signer_base64_io_t<T> &out, T **pos)
    {
        if (len > 2)
            return false;
    ...
    inline bool encode(MB_FS *mbfs, unsigned char *base64EncBuf, uint8_t *src, size_t len,
                       esp_signer_base64_io_t<T> &out, bool writeAllRemaining = true)
    {
    ...
    {
        esp_signer_base64_io_t<uint8_t> out;
        out.filetype = type;
    ...
    inline MB_String encodeToString(MB_FS *mbfs, uint8_t *src, size_t len)
    {
        ...
        esp_signer_base64_io_t<char> out;
        out.outT = encoded;
...
namespace HttpHelper
{
    ...
    inline void addGAPIsHost(MB_String &str, PGM_P sub)
    {
        str += sub;
        if (str[str.length() - 1] != '.')
            str += esp_signer_pgm_str_2; // "."
        str += esp_signer_pgm_str_3;     // "googleapis.com"
    }

    inline void addGAPIsHostHeader(MB_String &header, PGM_P sub)
    {
        header += esp_signer_pgm_str_4; // "Host: "
    ...
    {
        header += esp_signer_pgm_str_5; // "Content-Type: "
        header += v;
        header += esp_signer_pgm_str_1; // "\r\n"
    }
    ...
    {
        header += esp_signer_pgm_str_6; // "Content-Length: "
        header += len;
        addNewLine(header);
    }
    ...
    {
        header += esp_signer_pgm_str_7; // "User-Agent: ESP\r\n"
    }

    inline void addConnectionHeader(MB_String &header, bool keepAlive)
    {
        header += keepAlive ? esp_signer_pgm_str_8 /* "Connection: keep-alive\r\n" */
                            : esp_signer_pgm_str_9 /* "Connection: close\r\n" */;
    }

    /* Append the string with first request line (HTTP method) */
    inline bool addRequestHeaderFirst(MB_String &header, esp_signer_request_method method)
    {
        bool post = false;
        switch (method)
        {
        case http_get:
            header += esp_signer_pgm_str_10; // "GET"
            break;
        case http_post:
            header += esp_signer_pgm_str_11; // "POST"
            post = true;
            break;

        case http_patch:
            header += esp_signer_pgm_str_12; // "PATCH"
            post = true;
            break;

        case http_delete:
            header += esp_signer_pgm_str_13; // "DELETE"
            break;

        case http_put:
            header += esp_signer_pgm_str_14; // "PUT"
            break;
    ...
    {
        header += esp_signer_pgm_str_16; // " HTTP/1.1\r\n"
    }

    /* Append the string with first part of Authorization header */
    inline void addAuthHeaderFirst(MB_String &header)
    {
        header += esp_signer_pgm_str_17; // "Authorization: "
        header += esp_signer_pgm_str_18; // "Bearer "
    }

    inline void parseRespHeader(const MB_String &src, struct esp_signer_server_response_data_t &response)
    {
        int beginPos = 0;
        ...
        if (response.httpCode != -1)
        {
            ...
            StringHelper::tokenSubString(src, response.connection,
                                         esp_signer_pgm_str_20 /* "Connection: " */,
                                         esp_signer_pgm_str_1 /* "\r\n" */, beginPos, 0, false);
            StringHelper::tokenSubString(src, response.contentType,
                                         esp_signer_pgm_str_21 /* "Content-Type: " */,
                                         esp_signer_pgm_str_1 /* "\r\n" */, beginPos, 0, false);
            StringHelper::tokenSubStringInt(src, response.contentLen,
                                            esp_signer_pgm_str_22 /* "Content-Length: " */,
                                            esp_signer_pgm_str_1 /* "\r\n" */, beginPos, 0, false);
            StringHelper::tokenSubString(src, response.etag,
                                         esp_signer_pgm_str_23 /* "ETag: " */,
                                         esp_signer_pgm_str_1 /* "\r\n" */, beginPos, 0, false);
            response.payloadLen = response.contentLen;

            if (StringHelper::tokenSubString(src, response.transferEnc,
                                             esp_signer_pgm_str_24 /* "Transfer-Encoding: " */,
                                             esp_signer_pgm_str_1 /* "\r\n" */, beginPos, 0, false) &&
                StringHelper::compare(response.transferEnc, 0, esp_signer_pgm_str_25 /* "chunked" */))
                response.isChunkedEnc = true;

            if (response.httpCode == ESP_SIGNER_ERROR_HTTP_CODE_OK ||
                response.httpCode == ESP_SIGNER_ERROR_HTTP_CODE_TEMPORARY_REDIRECT ||
                response.httpCode == ESP_SIGNER_ERROR_HTTP_CODE_PERMANENT_REDIRECT ||
                response.httpCode == ESP_SIGNER_ERROR_HTTP_CODE_MOVED_PERMANENTLY ||
                response.httpCode == ESP_SIGNER_ERROR_HTTP_CODE_FOUND)
                StringHelper::tokenSubString(src, response.location,
                                             esp_signer_pgm_str_26 /* "Location: " */,
                                             esp_signer_pgm_str_1 /* "\r\n" */, beginPos, 0, false);
    ...
    inline int getStatusCode(const MB_String &header, int &pos)
    {
        ...
        StringHelper::tokenSubStringInt(header, code,
                                        esp_signer_pgm_str_27 /* "HTTP/1.1 " */,
                                        esp_signer_pgm_str_15 /* " " */, pos, 0, false);
        return code;
    }
    ...
    inline void intTCPHandler(GAuth_TCP_Client *client, struct esp_signer_tcp_response_handler_t &tcpHandler,
    ...
    inline bool readStatusLine(MB_FS *mbfs, Client *client, struct esp_signer_tcp_response_handler_t &tcpHandler,
    ...
    inline bool readHeader(MB_FS *mbfs, Client *client, struct esp_signer_tcp_response_handler_t &tcpHandler,
                           struct esp_signer_server_response_data_t &response)
    {
...
namespace Utils
{
    ...
    inline bool isNoContent(esp_signer_server_response_data_t *response)
    {
        return !response->isChunkedEnc && response->contentLen == 0;
    }

    inline bool isResponseTimeout(esp_signer_tcp_response_handler_t *tcpHandler, bool &complete)
    {
    ...
    inline bool isResponseComplete(esp_signer_tcp_response_handler_t *tcpHandler, esp_signer_server_response_data_t *response, bool &complete, bool check = true)
    {
    ...
    inline bool isChunkComplete(esp_signer_tcp_response_handler_t *tcpHandler, esp_signer_server_response_data_t *response, bool &complete)
    {
